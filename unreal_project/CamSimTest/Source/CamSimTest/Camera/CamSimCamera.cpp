...
void ACamSimCamera::CaptureAndEncode()
{
	...
	// Phase 26A: populate additional KLV tag fields
	CurrentTelemetry.FrameNumber = static_cast<uint32>(FrameIndex);
	CurrentTelemetry.TargetWidthM = (CurrentTelemetry.SlantRangeM > 0.0)
		? static_cast<float>(2.0 * CurrentTelemetry.SlantRangeM
			* FMath::Tan(FMath::DegreesToRadians(CurrentTelemetry.HFovDeg * 0.5f)))
		: 0.0f;
